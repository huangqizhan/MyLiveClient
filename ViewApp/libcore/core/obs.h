...
enum obs_bounds_type {
    ...
    OBS_BOUNDS_STRETCH,         /**< stretch (ignores base scale) source跟着边框的拉伸而拉伸 */
    OBS_BOUNDS_SCALE_INNER,     /**< scales to inner rectangle  source在边框内等比放缩 */
    OBS_BOUNDS_SCALE_OUTER,     /**< scales to outer rectangle  source在边框外等比放缩 */
    OBS_BOUNDS_SCALE_TO_WIDTH,  /**< scales to the width  source的宽度在边框内与边框一致  高等比放缩  */
...
#pragma mark --- 遍历 输入 输出 滤镜 动画各种类型
...
enum obs_deinterlace_mode {
	OBS_DEINTERLACE_MODE_DISABLE,//禁用去交错,不进行任何处理。
	OBS_DEINTERLACE_MODE_DISCARD,//丢弃隔行帧,仅使用完整的帧
	OBS_DEINTERLACE_MODE_RETRO, //使用简单的复制算法进行去交错。
	OBS_DEINTERLACE_MODE_BLEND, // 使用平均算法进行去交错,将相邻的隔行像素进行融合。
	OBS_DEINTERLACE_MODE_BLEND_2X,//与 BLEND 模式类似,但会进行两次融合操作。
	OBS_DEINTERLACE_MODE_LINEAR,  //使用线性插值算法进行去交错。
	OBS_DEINTERLACE_MODE_LINEAR_2X,//LINEAR 模式类似,但会进行两次线性插值操作。
	OBS_DEINTERLACE_MODE_YADIF,     //使用 YADIF (Yet Another Deinterlacer) 算法进行去交错。这是一种基于运动检测的算法,可以较好地保留细节和边缘。
	OBS_DEINTERLACE_MODE_YADIF_2X,//与 YADIF 模式类似,但会进行两次 YADIF 处理。
};
 ...
 * function!  Use obs_source_preload_video2 instead if partial range support is
 * desired for non-YUV video formats.
 * 立即显示frame 会设置到async_preload_frame上
 */
...
/**
 激活(video: play/stop filter:working/unworking )并显示child source
 */
EXPORT bool obs_source_add_active_child(obs_source_t *parent,
					obs_source_t *child);
///停止(video: play/stop filter:working/unworking )并隐藏child source
...
//把source中的audio buffer中的数据读到audio_mix中
...
/* ------------------------------------------------------------------------- */
/* Transition-specific functions  first ->second */
enum obs_transition_target {
	OBS_TRANSITION_SOURCE_A,  //first
	OBS_TRANSITION_SOURCE_B,  //second
};
...
enum obs_transition_mode {
	OBS_TRANSITION_MODE_AUTO,    ///自动计算transition_manual_val
	OBS_TRANSITION_MODE_MANUAL,  ///手动设置transition_manual_val 
};
///手动标记 转场开始
...
/**
 当obs已经设置transition_source
 此时就需要替换旧的transition_source
 */
...
EXPORT void obs_sceneitem_group_remove_item(obs_sceneitem_t *group,
					    obs_sceneitem_t *item);
//查找scene下的一个分组  item是要查找的组下的item
