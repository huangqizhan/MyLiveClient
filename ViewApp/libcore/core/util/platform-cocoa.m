...
/* gets the location   /Library/Application Support/ 存放插件*/
...
// /Library/Application Support/  存放插件
...
#pragma mark ---获取配置文件
/**
1 NSUserDomainMask:  此处存放配置文件
  Users/xxxx/Library/Application Support/
 
2 NSLocalDomainMask: 此处存放插件
  /Library/Application Support/

3 沙盒路径在Users/xxxx/Library/Containers/xxapp/  (signal capacity 添加了sandbox)
 
 */

//Users/xxx/Library/Application Support/[name]
...
static bool get_time_info(int64_t *cpu_time, int64_t *sys_time)
{
	...
	*sys_time = 0;
    ...
    ///TASK_THREAD_TIMES_INFO请求的是当前进程中线程的 CPU 使用时间信息（例如，用户时间、系统时间等）。
...
///===告诉系统当前应用请求更高的性能。这个操作可以让系统在某些情况下为你的应用分配更多的资源或优化其性能表现。
...
////"sysctl.proc_translated" 是一个在 macOS 系统中使用的 sysctl 参数，用于判断当前进程是否正在运行在 Apple Silicon（M1, M1 Pro, M1 Max, M2 等）芯片上通过 Rosetta 2 进行翻译的环境中。
...
bool os_get_proc_memory_usage(os_proc_memory_usage_t *usage)
{
	...
	usage->resident_size = taskinfo.resident_size;//实际物理内存大小
	usage->virtual_size = taskinfo.virtual_size;//虚拟内存大小 包括虚拟内存及交换内存
	return true;
}
