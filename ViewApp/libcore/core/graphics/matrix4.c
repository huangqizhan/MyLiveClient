...
///这个函数 get_3x3_submatrix 是用来从一个 4x4 的矩阵 m 中提取出一个 3x3 的子矩阵,并将其存储在 dst 指向的内存中。
