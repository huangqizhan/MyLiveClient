...
struct audio_output {
    ...
    /**
     录制文件时 音频轨可以有6个  每个轨代表一个mixer  每个轨可能包含多个channel
    */
...
///=====混音&&输出到编码器 audio_time当前tick的时间  prev_time上一次tick的时间
