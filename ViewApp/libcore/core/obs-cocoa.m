...
void add_default_module_paths(void)
{
	...
	if (abs_plugin_path != NULL) {
		...
		dstr_cat(&plugin_path, "/%module%.plugin/Contents/MacOS/");//bin
		dstr_cat(&plugin_data, "/%module%.plugin/Contents/Resources/");//data
...
///===用来返回当前 CPU 的品牌信息，比如 "Intel(R) Core(TM) i7-9750H CPU @ 2.60GHz"。这段代码的作用是通过 sysctlbyname 查询当前 CPU 的品牌信息并存储在 name 变量中
