...
void obs_output_set_preferred_size(obs_output_t *output, uint32_t width, uint32_t height)
{
	...
	if (output->info.flags & OBS_OUTPUT_ENCODED) {
		if (output->video_encoder)
			obs_encoder_set_scaled_size(output->video_encoder,width, height);
	}
}
///======
...
///======= 获取pkt所在的音频轨index
static size_t get_track_index(const struct obs_output *output, struct encoder_packet *pkt)
{
	for (size_t i = 0; i < MAX_OUTPUT_AUDIO_ENCODERS; i++) {
...
static bool add_caption(struct obs_output *output, struct encoder_packet *out)
{
	...
	if (output->caption_data.size > 0) {
		...
		sei_message_t *msg =
			sei_message_new(sei_type_user_data_registered_itu_t_t35,0, CEA608_MAX_SIZE);
...
static int prune_premature_packets(struct obs_output *output)
{
    ...
    ///duration_usec 代表视频帧 或当前音频帧的最长时间
    ///max_diff音频时间-视频时间的最大偏移量  超过帧最长时间
...
static bool initialize_interleaved_packets(struct obs_output *output)
{
	...
	/* ensure that there is audio past(超过) the first video packet */
	for (size_t i = 0; i < MAX_OUTPUT_AUDIO_ENCODERS; i++) {
		if (output->audio_encoders[i]) {
...
///==== （在没有开启延迟时 从编码器输出  音频视频的packet相互交错）/ (在开启延迟时 满足延迟时间的包)
			  ...
			  const struct audio_data *old, struct audio_data *new)
{
	...
	*new = *old;
	...
	if (old->timestamp < output->video_start_ts) {
		...
		uint64_t cutoff;
        ///音频帧数+帧数的开始时间 > 视频时间 此时不满足条件
		if (end_ts <= output->video_start_ts)
			return false;///丢弃
        
        ///视频时间必须在 [old->timestamp end_ts]之间
						 ...
						 bool signal)
{
	...
	if (delay_active(output)) {
		...
		if (!os_atomic_load_long(&output->delay_restart_refs)) {
            ...
            ///delay_restart_refs > 0 此处延迟重启 不需要开启destory线程 保持数据不丢失
			os_event_signal(output->stopping_event);
			return;
		}
	}
...
///====重连 （超时后才会重连即隔一段时间后再发起连接）
static void *reconnect_thread(void *param)
{
	struct obs_output *output = param;

	output->reconnect_thread_active = true;
    ///这里会阻塞当前线程 直到超时或者收到信号 才会继续
	if (os_event_timedwait(output->reconnect_stop_event, output->reconnect_retry_cur_msec) == ETIMEDOUT)
		obs_output_actual_start(output); ///超时时 再次重连

	if (os_event_try(output->reconnect_stop_event) == EAGAIN)
		pthread_detach(output->reconnect_thread);  ///超时
	else
		os_atomic_set_bool(&output->reconnecting, false); ///断开连接时如果还在重连状态  则发送重连信号 此时就不需要重连了

	output->reconnect_thread_active = false;
	return NULL;
}
///====output重连  此处开启新的线程发起连接  否则会阻塞
