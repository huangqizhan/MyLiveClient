...
///==== 把obs_scene_item插入到obs_scene(包括group_scene)下面的item的双向列表中
...
{
    ///水平方向 left默认0
    if (align & OBS_ALIGN_RIGHT)//右
        v->x += (float)cx;
    else if ((align & OBS_ALIGN_LEFT) == 0)//水平居中
        v->x += (float)(cx / 2);

    ///垂直方向 top默认0
    if (align & OBS_ALIGN_BOTTOM) //下
        v->y += (float)cy;
    else if ((align & OBS_ALIGN_TOP) == 0)//垂直居中
        v->y += (float)(cy / 2);
}
...
///====根据item的(放缩,平移,旋转) 更新scene_item的变换矩阵draw_transform/box_transform
...
///这个函数的作用是矩阵除了在水平垂直平移之外 是否没有其他的变换
...
static inline void render_item(struct obs_scene_item *item)
{
	...
	if (item->item_render &&
	    (!use_texrender || (gs_texrender_get_format(item->item_render) != format))) {
				    ...
				    obs_sceneitem_t *group_sceneitem)
{
	...
	while (item) {
		if (obs_source_removed(item->source)) {
			...
			remove_without_release(del_item);
			darray_push_back(sizeof(struct obs_scene_item *), remove_items, &del_item);
			rebuild_group = true;
			continue;
		}
...
///==== 获取obs_scene_t下的item列表中的idx位置的item对应的source
...
///====当拷贝一个新的obs_scene时 拷贝scene下的item对应的source
...
///遍历scene下的所有scene_item  当callback返回false时break
					       ...
					       obs_sceneitem_t *insert_after)
{
	...
	item->parent = scene;
	item->ref = 1;
    item->align = OBS_ALIGN_TOP | OBS_ALIGN_LEFT; //OBS_ALIGN_TOP | OBS_ALIGN_LEFT;
...
///==== load scene transform from temp
...
///==== 以json的形式加载scene下所有相关的transform
...
///==== 对比scene下的item列表顺序与items数组中的item顺序
...
///获取分组下的item在group_item->parent层面的位移 放缩 旋转 (也就是在group_item->parent空间中的transform) 即当前scent_item即将脱离group_item 归属到group_item->parent
static void get_ungrouped_transform(obs_sceneitem_t *group, struct vec2 *pos,
				    struct vec2 *scale, float *rot)
{
	struct matrix4 transform;
    ...
    ///此处的group->draw_transform是相对于group_item->parent的空间坐标的
	matrix4_copy(&transform, &group->draw_transform);
    ///变化pos
	transform_val(pos, &transform);
    ///意味着我们不对平移进行任何变动，平移量为零。
    ...
    //将缩放变换mat与之前的变换transform（旋转、平移等）结合起来，计算出最终的缩放矩阵。
...
///===把当前item的变换信息从原有的group_item空间移除 并且添加到group_item->parent空间中
...
static void apply_group_transform(obs_sceneitem_t *item, obs_sceneitem_t *group)
{
	...
	vec4_set(&x_base, 1.0f, 0.0f, 0.0f, 0.0f);
    ///逆矩阵   item->group_item  ==> item->group_item->parent
    ///       item->group_item->parent ==> item->group_item
...
///====更新scene下所有的item 在当前组和场景中 水平垂直方向的最大最小值
...
void obs_sceneitem_group_ungroup(obs_sceneitem_t *item)
{
	...
	full_lock(subscene);
    ///group_scene下的第一个item
	first = subscene->first_item;
	last = first;
    ///此处的分组下的旧的item列表此处无需释放 在UI层会处理
	while (last) {
		obs_sceneitem_t *dst;
				     ...
				     obs_sceneitem_t *item)
{
    ...
    ///把item从group中移除 再加入到与group同级的列表中
	detach_sceneitem(item);
	attach_sceneitem(scene, item, NULL);
...
///把当前scene下的 scene_item（包括group_item下的item）存放到items_out中
...
///===当前scene下的所有item顺序跟items顺序是否不一致
...
///===按照obs_sceneitem_order_info重新排序
