	       ...
	       obs_data_t *settings, size_t mixer_idx, obs_data_t *hotkey_data)
{
    ...
    ///把encoder插入到obs->data.first_encoder链表
...
///====== 删除video_output audio_output的输入编码器的回调
					      ...
					      size_t sample_rate)
{
	...
	if (pause->ts_start >= data->timestamp) {
        ...
        ///此处只要pause->ts_start > 0  pause->ts_start < data->timestamp 符合丢弃的条件
		return true;
	}
