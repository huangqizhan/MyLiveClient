...
/*
 image_source->source->context.data == image_source
...
static const char *image_source_get_name(void *unused)
{
	...
	const char*name = obs_module_text("ImageInput");
    return name;
}
...
static void image_source_render(void *data, gs_effect_t *effect)
{
    ...
    ///关联effect的参数跟纹理数据 （参数是effect中image字段(uniform texture2d image)  纹理中存放图片数据）
