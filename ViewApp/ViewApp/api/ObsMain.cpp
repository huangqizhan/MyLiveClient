...
cmp = extensions; \
while (*cmp) { \
    if (astrcmpi(*cmp, suffix.c_str()) == 0) { \
...
bool ObsMain::LoadScene(){
    ...
    ret = snprintf(fileName, 512, "obs-studio1/basic/scenes/%s.json",
        sceneCollection);
    if (ret <= 0){
...
void ObsMain::ClearSceneData(){
    ...
    auto cb = [](void *unused, obs_source_t *source)->bool
    {
        obs_source_remove(source);
...
void ObsMain::CreateDefaultScene(bool firstStart){
    ...
    obs_scene_t  *scene = obs_scene_create(ToUtf8(L"scene",5).c_str());
    if (firstStart)
        InitAudioSources();
...
void ObsMain::LoadSceneListOrder(obs_data_array_t *array){
    ...
    for (size_t i = 0; i < num; i++) {
        ...
        for (auto& sdata : scenes){
            if (data && strcmp(sdata->name(), name) == 0){
                m_scenes.push_back(std::move(sdata));
                break;
            }
        }
        obs_data_release(data);
...
static obs_data_t *GenerateSaveData(config_t* config,obs_data_array_t *sceneOrder,
    ...
    /* -------------------------------- */
    /* save non-group sources  non-output source  */
    ...
    obs_data_array_t *sourcesArray = obs_save_sources_filtered([](void *data, obs_source_t *source){
    ...
    obs_data_array_t *groupsArray = obs_save_sources_filtered( [](void*, obs_source_t *source){
...
void ObsMain::Save(const char *file){
    ...
    obs_data_t *saveData = GenerateSaveData(globalConfig(), sceneOrder, scene, 300);
