...
#define DL_OPENGL "liblibobs-opengl.dylib"
#define DL_D3D11 "obs_d3d11"

ObsBasic::ObsBasic()
{
    char path[512];
    int ret = GetConfigPath(path, 512, "obs-studio1/plugin_config");
    if (ret <= 0)
        path[0]='\0';
    blog(LOG_INFO, "config path = %s",path);
...
bool ObsBasic::ResetAudio()
{
    ...
    ai.samples_per_sec = (uint32_t)config_get_uint(m_basicConfig, "Audio",
        "SampleRate");
...
void ObsBasic::SetTransition(OBSSource transition)
{
    ...
    ///如果之前已经设置过了transition_source 则需要swap切换transition_source
    if (oldTransition && transition) {
        obs_transition_swap_begin(transition, oldTransition);
        obs_set_output_source(0, transition);
        obs_transition_swap_end(transition, oldTransition);
    }else {
        ///如果没有设置过transition_source 则直接设置 无需swap
        obs_set_output_source(0, transition);
    }
...
int ObsBasic::GetProfilePath(char *path, size_t size, const char *file){
    ...
    ret = GetConfigPath(profiles_path, 512, "obs-studio1/basic/profiles");
    if (ret <= 0)
        return ret;
...
bool ObsBasic::MakeUserDirs(){
    ...
    if (GetConfigPath(path, sizeof(path), "obs-studio1/basic") <= 0)
        return false;
    if (!do_mkdir(path))
        return false;

    if (GetConfigPath(path, sizeof(path), "obs-studio1/logs") <= 0)
        return false;
    if (!do_mkdir(path))
        return false;

    if (GetConfigPath(path, sizeof(path), "obs-studio1/profiler_data") <= 0)
        return false;
    if (!do_mkdir(path))
        return false;

#ifdef _WIN32
    if (GetConfigPath(path, sizeof(path), "obs-studio1/crashes") <= 0)
        return false;
    if (!do_mkdir(path))
        return false;

    if (GetConfigPath(path, sizeof(path), "obs-studio1/updates") <= 0)
        return false;
    if (!do_mkdir(path))
        return false;
#endif

    if (GetConfigPath(path, sizeof(path), "obs-studio1/plugin_config") <= 0)
        return false;
    if (!do_mkdir(path))
        return false;
...
bool ObsBasic::MakeUserProfileDirs(){
    ...
    if (GetConfigPath(path, sizeof(path), "obs-studio1/basic/profiles") <= 0)
        return false;
    if (!do_mkdir(path))
        return false;

    if (GetConfigPath(path, sizeof(path), "obs-studio1/basic/scenes") <= 0)
        return false;
    if (!do_mkdir(path))
        return false;
