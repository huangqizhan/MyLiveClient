...
void SimpleOutput::LoadRecordingPreset(){
    ...
    if (strcmp(quality, "Stream") == 0) {///推流  有损压缩
        ...
        if (!CreateAACEncoder(aacRecordingEncoder, aacRecEncID, 192, "simple_aac_recording", 0))
...
SimpleOutput::SimpleOutput(ObsBasic *main_) : BasicOutputHandler(main_)
{
    ...
    ///aac
    if (!CreateAACEncoder(aacStreamingEncoder, aacStreamEncID, GetAudioBitrate(),
                "simple_aac", 0))
...
void SimpleOutput::Update(){
    ...
    if (format != VIDEO_FORMAT_NV12 && format != VIDEO_FORMAT_I420)
        obs_encoder_set_preferred_video_format(h264StreamingEncoder,
                VIDEO_FORMAT_NV12);

    obs_encoder_update(h264StreamingEncoder, h264Settings);
    obs_encoder_update(aacStreamingEncoder,  aacSettings);
...
void SimpleOutput::UpdateRecordingAudioSettings(){
    ...
    obs_encoder_update(aacRecordingEncoder, settings);
...
void SimpleOutput::UpdateRecordingSettings_x264_crf(int crf){
    ...
    obs_encoder_update(h264RecordingEncoder, settings);
...
inline void SimpleOutput::SetupOutputs(){
    ...
    if (usingRecordingPreset) {
        if (ffmpegOutput) {
            ...
            obs_encoder_set_video(h264RecordingEncoder, obs_get_video());
            obs_encoder_set_audio(aacRecordingEncoder,  obs_get_audio());
        }
    }
}
...
bool SimpleOutput::StartStreaming(obs_service_t *service){
    ...
    if (outputType != type) {
        ...
        if (strcmp(codec, "aac") != 0) {
            ...
            aacStreamingEncoder = obs_audio_encoder_create(id,
                    "alt_audio_enc", nullptr, 0, nullptr);
            obs_encoder_release(aacStreamingEncoder);
            if (!aacStreamingEncoder)
                return false;

            obs_encoder_update(aacStreamingEncoder, settings);
            obs_encoder_set_audio(aacStreamingEncoder, obs_get_audio());
    ...
    obs_output_set_video_encoder(streamOutput, h264StreamingEncoder);
    obs_output_set_audio_encoder(streamOutput, aacStreamingEncoder, 0);
...
void SimpleOutput::UpdateRecording(){
    ...
    if (!ffmpegOutput) {
        obs_output_set_video_encoder(fileOutput, h264RecordingEncoder);
        obs_output_set_audio_encoder(fileOutput, aacRecordingEncoder, 0);
    }
    if (replayBuffer) {
        obs_output_set_video_encoder(replayBuffer, h264RecordingEncoder);
        obs_output_set_audio_encoder(replayBuffer, aacRecordingEncoder, 0);
    }
